...
static inline void fac_mini_tests(fac_params *m)
{
	...
	qs_sm seconds = bits < 140 ? 30 : bits < 160 ? 60
								  : bits < 180	 ? 120
												 : 180,
		  trial_max;
	const char *seconds_str = bits < 140 ? "30 seconds" : bits < 160 ? "minute"
													  : bits < 180	 ? "2 minutes"
																	 : "3 minutes";
	if (m->testing >= 3)
		printf("-- %3d-bit : your %s factorization test -- \n\n", bits, seconds_str);
	else
		printf("-- crescendo : your %s factorization test -- \n\n", seconds_str);
	...
	retry:
		...
		for (int i = 0; factors[i]; ++i)
		{
			...
			case 0:
				...
				break;
			case 1:
				printf(factors[i + 1] ? "%s * " : "%s", str);
				break;
			default:
				printf(factors[i + 1] ? "(%s ^ %d) * " : "(%s ^ %d)", str, powers);
				break;
			}
			free(str);
			fflush(stdout);
			const int n_bits = (int)cint_count_bits(&factors[i]->cint);
			if (n_bits != factors[i]->bits)
				error_number |= 4;
	...
	if (error_number == 0 && m->testing >= 3)
		puts("Prime numbers wasn't submitted, the software has checked the answers.");
	if (error_number & 2)
		puts("number wasn't a multiple of the factor");
	if (error_number & 4)
		puts("bit count of the factor was wrong");
	if (error_number & 8)
		puts("factor of number wasn't 'prime'");
	if (error_number & 16)
		puts("number wasn't correctly factored");
	if (error_number)
		printf("qs rand seed was %u\n", params.qs_rand_seed);
